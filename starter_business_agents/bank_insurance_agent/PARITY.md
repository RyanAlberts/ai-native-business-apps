...
captured live after the prompt was updated to disambiguate the two
"beneficial ownership" concepts — the bank's FinCEN CDD onboarding form
(relevant here) vs. the separate CTA BOI report (US-formed entities exempt
under the 2025 rule). The new baseline frames the bank CDD step correctly
and no longer tells founders to file a BOI report. The prior
`tests/baselines/claude-2026-05-12.md` (5 cases) is kept as the historical
pre-correction snapshot. Re-capture the full set with `python
scripts/parity_run.py starter_business_agents.bank_insurance_agent`.
