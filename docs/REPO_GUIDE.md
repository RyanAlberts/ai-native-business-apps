# Repo Guide — developing Keel

Context for humans and AI coding agents **working on Keel itself** (new
agents, provider adapters, prompt edits, core changes). If you're here to
*run* the Founding Journey for a founder, this file is not for you — the
runner contract in the repo root's `CLAUDE.md` / `AGENTS.md` is.
...
├── brand.py        # Product name + DISCLAIMER in ONE place (slug unchanged)
...
playbooks/          # Agent-native Founding Journey (no Python needed):
verify/             # markdown playbooks + deterministic gate script that let
                    # Claude Code / Codex / Cursor run the journey directly
```
...
`run()`. The agent-native path (`playbooks/`) reuses the same single
sources of truth: each playbook points the running agent at the specialist's
`prompts.py`, `core/state_portals.py`, and the deterministic tool logic in
`tools.py` instead of duplicating them. **If you edit a specialist prompt or
a tool's semantics, check the corresponding playbook still describes it
truthfully.**
...
The SessionStart hook (`.claude/settings.json` → `scripts/dev_setup.sh`)
provisions a `.venv` with `claude-agent-sdk` + pytest + ruff, but only for
contributors who opted in — fresh clones skip it so founders running the
agent-native journey never sit through a pip install. Opt in once with:

```bash
KEEL_DEV=1 bash scripts/dev_setup.sh
```

After that the hook keeps the existing `.venv` current on every session.
...
- If you touched a specialist prompt, tool, `core/state_portals.py`, or
  `core/brand.py::DISCLAIMER`: re-read the matching `playbooks/*.md` file —
  the agent-native path references these files by name and by behavior.
...
- Don't duplicate specialist prompt content into `playbooks/` — the
  playbooks deliberately *reference* `prompts.py` / `tools.py` /
  `state_portals.py` so there is one source of truth per domain.
